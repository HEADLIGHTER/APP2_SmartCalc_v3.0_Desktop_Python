import tkinter as tk
import matplotlib.pyplot as pl
import src.view_model.data as data
import src.view_model.vm_utils as vm_ut
from src.view_model.check_parse import check_parse, x_func


pl.rcParams['figure.figsize'] = [7.30, 3.70]
pl.rcParams['figure.autolayout'] = True


root = tk.Tk()
root.geometry("500x500")


def helping():
	helptext = tk.Tk()
	helptext.geometry('370x370')
	helptext = tk.Label(helptext, text=data.help_label, font='Px\\ IBM\\ EGA8 15', justify='left')
	helptext.pack()
	helptext.place()

def temp_text(e):
	ent37.delete(0, 'end')


def button_click(text, ent):
	if text == 'C':
		ent.delete(0, tk.END)
	elif text == 'tan':
		ent.insert(tk.END, 'tan(')
	elif text in 'sqrtatanacosasinalnlog':
		ent.insert(tk.END, text + '(')
	elif text == '<-':
		ent.delete(len(ent.get())-1)
	elif text == 'Quit':
		exit(1337)
	elif text == 'mem':
		hist()
	elif text == 'cmem':
		vm_ut.clear_history()
		data.hi_list.clear()
	elif text == '=':
		calc_to_ui(text)
	elif text == 'Help':
		helping()
	else:
		ent.insert(tk.END, text)


def hist():
	try:
		if data.hi_list[data.curr_h]:
			ent13.delete(0, tk.END)
			ent13.insert(0, data.hi_list[data.curr_h])
		data.curr_h += 1
	except IndexError:
		pass


def graph(c_x: list, c_y: list):
	fig, ax = pl.subplots()
	ax.spines['left'].set_position('center')
	ax.spines['bottom'].set_position('center')
	ax.spines['right'].set_color('none')
	ax.spines['top'].set_color('none')
	ax.xaxis.set_ticks_position('bottom')
	ax.yaxis.set_ticks_position('left')
	ax.grid('on')
	pl.plot(c_x, c_y, 'b')
	pl.show()


def calc_to_ui(event):
	vm_ut.save_history(str(ent13.get()))
	vm_ut.load_history()
	exp = '1*' + ent13.get()
	exp = exp.lower()
	x_val = ent37.get()
	x_val = x_val.lower()
	if len(x_val) > 0:
		exp = exp.replace('x', ent37.get())
	if exp.find('x') >= 0:
		c_x, c_y = x_func(exp.replace(' ', ''))
		graph(c_x, c_y)
	else:
		res = check_parse(exp)
		ent13.delete(0, tk.END)
		ent13.insert(0, res)


root.bind('<Return>', calc_to_ui)

#  interface
ent13 = tk.Entry(font='Px\\ IBM\\ EGA8 37')
ent37 = tk.Entry(font='Px\\ IBM\\ EGA8 37', justify='center')


ent37.insert(0, 'x')
ent37.bind('<FocusIn>', temp_text)

ent13.place(width=1837, height=137, y=370, x=137)
ent37.place(width=137, height=137, y=370)


buttons = [
	{"text": "1", "row": 5, "column": 0},
	{"text": "2", "row": 5, "column": 1},
	{"text": "3", "row": 5, "column": 2},
	{"text": "4", "row": 4, "column": 0},
	{"text": "5", "row": 4, "column": 1},
	{"text": "6", "row": 4, "column": 2},
	{"text": "7", "row": 3, "column": 0},
	{"text": "8", "row": 3, "column": 1},
	{"text": "9", "row": 3, "column": 2},
	{"text": "0", "row": 6, "column": 0},
	{"text": "/", "row": 6, "column": 3},
	{"text": "*", "row": 5, "column": 3},
	{"text": "-", "row": 4, "column": 3},
	{"text": "+", "row": 3, "column": 3},
	{"text": ".", "row": 6, "column": 1},
	{"text": "=", "row": 7, "column": 1},
	{"text": "C", "row": 6, "column": 2},
	{"text": "(", "row": 3, "column": 4},
	{"text": ")", "row": 3, "column": 5},
	{"text": "sin", "row": 4, "column": 4},
	{"text": "cos", "row": 4, "column": 5},
	{"text": "tan", "row": 6, "column": 5},
	{"text": "acos", "row": 5, "column": 5},
	{"text": "asin", "row": 5, "column": 4},
	{"text": "atan", "row": 7, "column": 5},
	{"text": "sqrt", "row": 6, "column": 4},
	{"text": "ln", "row": 7, "column": 4},
	{"text": "log", "row": 7, "column": 3},
	{"text": "<-", "row": 7, "column": 2},
	{"text": "Help", "row": 7, "column": 7},
	{"text": "Quit", "row": 7, "column": 0},
	{"text": "mem", "row": 5, "column": 7},
	{"text": "cmem", "row": 6, "column": 7},
	{"text": "x", "row": 4, "column": 7},
	{"text": "pi", "row": 3, "column": 7}
]
for button in buttons:
	command = lambda x=button["text"]: button_click(x, ent13)
	tk.Button(
		text=button["text"],
		width=3,
		height=2,
		bd=5,
		bg="#F0F0F0",
		command=command
	).grid(
		row=button["row"],
		column=button["column"],
		padx=7,
		pady=7,
		columnspan=button.get("columnnspan", 1),
		sticky="nsew"
	)


vm_ut.load_history()

root.mainloop()
